...
section [1.2.1 Testing](pintos_1.html#SEC8)).
...
:   A simple manager for 80`x`{.variable}86 hardware page tables.
    ...
    [4.1.2.3 Page Tables](pintos_4.html#SEC59), for more information.
...
:   When a user process performs a privileged or prohibited operation,
    ...
    \"fault.\"[(3)](pintos_fot.html#FOOT3){#DOCF3} These files handle
...
-   Unix-like semantics for `filesys_remove()` are implemented. That is,
    ...
    File](pintos_3.html#Removing%20an%20Open%20File), for more
    information.
...
"`-a newname`{.sample}": "pintos -p `file`{.variable} -a
`newname`{.variable} \-- -q". The commands for copying files out of a VM
are similar, but substitute "`-g`{.sample}" for "`-p`{.sample}".
...
+-----------------------------------+-----------------------------------+
|                                   |     pintos-mk                     |
|                                   | disk filesys.dsk --filesys-size=2 |
|                                   |     pintos -f -q                  |
|                                   |     pintos -p                     |
...
approximately `n`{.variable} megabytes in size just for the duration of
the `pintos` run. The Pintos automatic test suite makes extensive use of
this syntax:

+-----------------------------------+-----------------------------------+
|                                   |     pintos --f                    |
|                                   | ilesys-size=2 -p ../../examples/e |
|                                   | cho -a echo -- -f -q run 'echo x' |
+-----------------------------------+-----------------------------------+
...
specified in \[ [SysV-i386](pintos_13.html#SysV-i386)\] and has no deep
significance.
...
+-----------------------------------+-----------------------------------+
|                                   |     /* Reads a by                 |
|                                   | te at user virtual address UADDR. |
|                                   |                                   |
|                                   |    UADDR must be below PHYS_BASE. |
|                                   |        Returns the byte valu      |
|                                   | e if successful, -1 if a segfault |
|                                   |        occurred. */               |
|                                   |     static int                    |
|                                   |                                   |
...
-   Argument passing (see section [3.3.3 Argument
    Passing](pintos_3.html#SEC44)). Every user program will page fault
...
-   User memory access (see section [3.1.5 Accessing User
    Memory](pintos_3.html#SEC39)). All system calls need to read user
    memory. Few system calls need to write to user memory.

-   System call infrastructure (see section [3.3.4 System
    Calls](pintos_3.html#SEC45)). Implement enough code to read the
...
See section [3.5.1 Program Startup Details](pintos_3.html#SEC51), for
...
:   Deletes the file called `file`{.variable}. Returns true if
    ...
    File](pintos_3.html#Removing%20an%20Open%20File), for details.
...
functionality. See section [3.1.5 Accessing User
Memory](pintos_3.html#SEC39), for more information.
...
See section [3.5.2 System Call Details](pintos_3.html#SEC52), for
...
:   You\'ll have to implement system calls before you see anything else.
    ...
    that argument passing is implemented correctly (see section [3.5.1
    Program Startup Details](pintos_3.html#SEC51)).
...
:   The `objdump` (80`x`{.variable}86) or `i386-elf-objdump` (SPARC)
    ...
    disassemble individual functions (see section [E.5
    GDB](pintos_10.html#SEC151)).
...
details, refer to \[ [SysV-i386](pintos_13.html#SysV-i386)\].
...
[3.5 80`x`{.variable}86 Calling Convention](pintos_3.html#SEC50)).
...
0xc0000000:
  ...
  0xbffffffc   `argv[3][...]`   "`bar\0`{.sample}"       `char[4]`
  0xbffffff8   `argv[2][...]`   "`foo\0`{.sample}"       `char[4]`
  0xbffffff5   `argv[1][...]`   "`-l\0`{.sample}"        `char[3]`
  0xbfffffed   `argv[0][...]`   "`/bin/ls\0`{.sample}"   `char[8]`
...
Handling](pintos_6.html#SEC109)). These can be errors such as a page
...
interrupt (see section [3.5 80`x`{.variable}86 Calling
Convention](pintos_3.html#SEC50)).
