...
you did in the previous assignment (see section [3.1.2 Using the File
System](pintos_3.html#SEC35)).
...
new "Makefile" turns on the setting "`-DVM`{.sample}". All code you
...
2 (see section [3.1.4 Virtual Memory Layout](pintos_3.html#SEC37)), but
much of it is new.
...
access only its own user pages. See section [3.1.4 Virtual Memory
Layout](pintos_3.html#SEC37), for more information.

Pintos provides several useful functions for working with virtual
addresses. See section [A.6 Virtual Addresses](pintos_6.html#SEC114),
for details.
...
Addresses](pintos_6.html#SEC114), for details.
...
:   Enables page fault handling by supplementing the hadrware page
    table. See section [4.1.4 Managing the Supplemental Page
    Table](pintos_4.html#SEC62).

Frame table

:   Allows efficient implementation of eviction policy. See section
    [4.1.5 Managing the Frame Table](pintos_4.html#SEC63).

Swap table

:   Tracks usage of swap slots. See section [4.1.6 Managing the Swap
    Table](pintos_4.html#SEC65).
...
Table](pintos_6.html#SEC123)). Pintos hash tables efficiently support
...
[A.7.4.2 Page Table Entry Format](pintos_6.html#SEC121), for more
information.
...
2.  Obtain a frame to store the page. See section [4.1.5 Managing the
    Frame Table](pintos_4.html#SEC63), for details.
...
test cases to fail unexpectedly (see [Why
PAL_USER?](pintos_4.html#Why%20PAL_USER?)). If you modify "palloc.c" as
...
credit (see [VM Extra Credit](pintos_4.html#VM%20Extra%20Credit)), or if
there is a bug in your code.

See section [A.7.3 Accessed and Dirty Bits](pintos_6.html#SEC118), for
...
"swap.dsk" that contains a `n`{.variable}-MB swap partition. Afterward,
...
`n`{.variable}-MB swap disk for a single run with
"`--swap-size=n`{.sample}".
...
Suppose file "foo" is 0x1000 bytes (4 kB, or one page) long. If "foo" is
mapped into memory starting at address 0x5000, then any memory accesses
to locations 0x5000[\...]{.small}0x5fff will access the corresponding
bytes of "foo".
...
0x10000000, writes the mapped data to the console (fd 1), and unmaps the
file.
...
1.  Frame table (see section [4.1.5 Managing the Frame
    Table](pintos_4.html#SEC63)). Change "process.c" to use your frame
    table allocator.
...
2.  Supplemental page table and page fault handler (see section [4.1.4
    Managing the Supplemental Page Table](pintos_4.html#SEC62)). Change
    "process.c" to record the necessary information in the supplemental
...
Frame Table](pintos_4.html#SEC63)). Initially you could choose the page
...
document template](vm.tmpl) into your source tree under the name
...
section [D. Project Documentation](pintos_9.html#SEC142), for a sample
design document that goes along with a fictitious project.
...
stack.[(4)](pintos_fot.html#FOOT4){#DOCF4} However, the
80`x`{.variable}86 `PUSH` instruction checks access permissions before
...
section [3.1.5 Accessing User Memory](pintos_3.html#SEC39)), these are
the only cases you need to handle. On the other hand, if you depend on
...
[System Call:]{.underline} mapid_t **mmap** (int `fd`{.variable}, void \*`addr`{.variable})

:   Maps the file open as `fd`{.variable} into the process\'s virtual
    address space. The entire file is mapped into consecutive virtual
    pages starting at `addr`{.variable}.
    ...
    A call to `mmap` may fail if the file open as `fd`{.variable} has a
    length of zero bytes. It must fail if `addr`{.variable} is not
    ...
    executable load time. It must also fail if `addr`{.variable} is 0,
...
[System Call:]{.underline} void **munmap** (mapid_t `mapping`{.variable})
:   Unmaps the mapping designated by `mapping`{.variable}, which must be
...
File](pintos_3.html#Removing%20an%20Open%20File), for more information.
...
[3.1.5 Accessing User Memory](pintos_3.html#SEC39)) while handling a
...
:   Here\'s a summary of our reference solution, produced by the
    ...
    project 2. See section [3.4 FAQ](pintos_3.html#SEC47), for the
...
:   Returning from `page_fault()` resumes the current user process (see
    section [A.4.2 Internal Interrupt Handling](pintos_6.html#SEC109)).
