...
////
 char *filename, *save_ptr;
  ...
  tid = thread_create (filename, thread_current()->priority,
...
start_process (void *file_name_)
{
  ...
  /* If load failed, quit. */
  // palloc_free_page (file_name); old

  palloc_free_page (pg_round_down(file_name)); //new 
...
{
  const char *file_name_ = thread_name();
  struct thread *t = thread_current ();
...
void push_argument(void **esp, const char *arg, struct list *list) 
{
  int len = strlen(arg) + 1;
  *esp -= len;

  struct arg_addr *addr = malloc(sizeof(struct arg_addr));
  memcpy(*esp, arg, len);
...
setup_stack (void **esp, const char* file_name) 
{
  ...
  * (uint32_t *) *esp = (uint32_t) NULL;
  ...
  while (!list_empty(&list)) {
    struct arg_addr *addr = 
      list_entry(list_pop_back(&list), struct arg_addr, list_elem);
    *esp -= 4;
